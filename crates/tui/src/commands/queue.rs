...
pub fn queue(app: &mut App, args: Option<&str>) -> CommandResult {
    ...
    match action.as_str() {
        ...
        _ => CommandResult::error(app.tr(MessageId::CmdQueueUsage)),
    }
}
...
fn list_queue(app: &mut App) -> CommandResult {
    ...
    if let Some(draft) = app.queued_draft.as_ref() {
        lines.push(app.tr(MessageId::CmdQueueEditingHeader).to_string());
        lines.push(format!("- {}", truncate_preview(&draft.display)));
    }

    if queued == 0 {
        if lines.is_empty() {
            return CommandResult::message(app.tr(MessageId::CmdQueueNoMessages));
        }
        return CommandResult::message(lines.join("\n"));
    }

    lines.push(
        app.tr(MessageId::CmdQueueMessagesHeader)
    ...
    lines.push(app.tr(MessageId::CmdQueueTip).to_string());
...
fn edit_queue(app: &mut App, index: Option<&str>) -> CommandResult {
    if app.queued_draft.is_some() {
        return CommandResult::error(app.tr(MessageId::CmdQueueAlreadyEditing));
    }
    let index = match parse_index(index) {
        Ok(index) => index,
        Err(id) => return CommandResult::error(app.tr(id)),
    };

    let Some(message) = app.remove_queued_message(index) else {
        return CommandResult::error(app.tr(MessageId::CmdQueueNotFound));
    };
    ...
    app.status_message = Some(
        app.tr(MessageId::CmdQueueEditingStatus)
            .replace("{index}", &(index + 1).to_string()),
    );

    CommandResult::message(
        app.tr(MessageId::CmdQueueEditingMessage)
            .replace("{index}", &(index + 1).to_string()),
    )
}
...
fn drop_queue(app: &mut App, index: Option<&str>) -> CommandResult {
    ...
    CommandResult::message(
        app.tr(MessageId::CmdQueueDropped)
            .replace("{index}", &(index + 1).to_string()),
    )
}
...
fn clear_queue(app: &mut App) -> CommandResult {
    ...
    if queued == 0 && !had_draft {
        return CommandResult::message(app.tr(MessageId::CmdQueueAlreadyEmpty));
    }

    CommandResult::message(app.tr(MessageId::CmdQueueCleared))
}

fn parse_index(input: Option<&str>) -> Result<usize, MessageId> {
    let Some(input) = input else {
        return Err(MessageId::CmdQueueMissingIndex);
    };
    let raw = input
        .parse::<usize>()
        .map_err(|_| MessageId::CmdQueueIndexPositive)?;
    if raw == 0 {
        return Err(MessageId::CmdQueueIndexMin);
    }
    Ok(raw - 1)
}
